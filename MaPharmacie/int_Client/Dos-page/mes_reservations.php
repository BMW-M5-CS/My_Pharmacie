...
// ===== Regroupement PHP =====
// Tant qu'une demande n'est pas confirmée, il n'y a pas encore de
// code_reservation (il n'existe qu'à la confirmation par le pharmacien) —
// on regroupe alors par "groupe_demande" à la place.

$groupes = [];

foreach ($lignes as $ligne) {
    $cle = $ligne['code_reservation'] ?: $ligne['groupe_demande'];
    if (!isset($groupes[$cle])) {
        $groupes[$cle] = [
            'code_reservation'       => $ligne['code_reservation'],
            'groupe_demande'         => $ligne['groupe_demande'],
            'statut'                 => $ligne['statut'],
            'date_reservation'       => $ligne['date_reservation'],
            'expire_at'              => $ligne['expire_at'],
            'expire_demande_at'      => $ligne['expire_demande_at'],
            'renouvelable'           => $ligne['renouvelable'],
...
$compteurs = ['demandee' => 0, 'confirmee' => 0, 'rejetee' => 0, 'annulee' => 0, 'expiree' => 0, 'renouvele' => 0];
...
$badges = [
    'demandee'   => ['label' => 'En attente de validation', 'classe' => 'badge-attente'],
    'confirmee'  => ['label' => 'Confirmée',                 'classe' => 'badge-confirme'],
    'rejetee'    => ['label' => 'Rejetée',                   'classe' => 'badge-annule'],
...
<body>
    ...
    <div class="mresa-filtres">
        ...
        <button class="filtre-btn" data-filtre="demandee">
            En attente <span class="filtre-compteur"><?php echo $compteurs['demandee']; ?></span>
        </button>
        ...
        <button class="filtre-btn" data-filtre="rejetee">
            Rejetées <span class="filtre-compteur"><?php echo $compteurs['rejetee']; ?></span>
        </button>
    ...
    <section class="mresa-contenu" id="mresa-liste">
        ...
        <?php else : ?>
            ...
            <?php foreach ($groupes as $groupe) :
                ...
                $expire_fmt     = $groupe['expire_at'] ? date('d/m/Y à H:i', strtotime($groupe['expire_at'])) : null;
                $expire_demande_fmt = $groupe['expire_demande_at'] ? date('d/m/Y à H:i', strtotime($groupe['expire_demande_at'])) : null;
            ?>
                    ...
                    <div class="mresa-carte-head">
                        ...
                        <div class="mresa-head-left">
                            ...
                            <span class="mresa-code">
                                ...
                                <?php echo $groupe['code_reservation']
                                    ? htmlspecialchars($groupe['code_reservation'])
                                    : 'Code communiqué après acceptation'; ?>
                            </span>
                   ...
                   <!-- Produits en grille -->
                    ...
                    <div class="mresa-carte-foot">
                        ...
                        <div class="mresa-dates">
                            ...
                            <?php if ($groupe['statut'] === 'demandee') : ?>
                                <span class="mresa-date-ligne mresa-expire-info">
                                    <i class="fa-solid fa-hourglass-half"></i>
                                    La pharmacie doit répondre avant le <?php echo $expire_demande_fmt; ?>
                                </span>

                            <?php elseif ($groupe['statut'] === 'confirmee') : ?>
                                <span class="mresa-date-ligne mresa-expire-info">
                                    <i class="fa-solid fa-hourglass-half"></i>
                                    À retirer avant le <?php echo $expire_fmt; ?>
                                </span>

                            <?php elseif ($groupe['statut'] === 'rejetee') : ?>
                                <span class="mresa-date-ligne mresa-rejet-info">
                                    <i class="fa-solid fa-circle-exclamation"></i>
                                    <?php echo $groupe['motif_rejet']
                                        ? 'Motif : ' . htmlspecialchars($groupe['motif_rejet'])
                                        : 'La pharmacie n\'a pas donné de motif.'; ?>
                                </span>
                            ...
                            <?php elseif ($groupe['statut'] === 'renouvele' && !empty($groupe['reserve_code_renouvele'])) : ?>
                                <span class="mresa-date-ligne mresa-renouvele-info">
                                    ...
                                    <?php if (str_starts_with($groupe['reserve_code_renouvele'], 'RES-')) : ?>
                                        Renouvelée vers <?php echo htmlspecialchars($groupe['reserve_code_renouvele']); ?>
                                    <?php else : ?>
                                        Nouvelle demande envoyée, en attente de validation par la pharmacie
                                    <?php endif; ?>
                                </span>
                        ...
                        <?php if (in_array($groupe['statut'], ['demandee', 'confirmee'], true)) : ?>
                                <button
                                    class="mresa-btn-annuler"
                                    data-code="<?php echo htmlspecialchars($groupe['code_reservation'] ?? ''); ?>"
                                    data-groupe="<?php echo htmlspecialchars($groupe['groupe_demande'] ?? ''); ?>">
                            ...
                            <?php elseif ($groupe['statut'] === 'expiree') : ?>
                                ...
                                <?php
                                    ...
                                    // pharmacien n'a pas répondu (expire_demande_at), soit le client n'est
                                    // pas venu chercher après confirmation (expire_at).
                                    $reference_expiration = $groupe['expire_at'] ?: $groupe['expire_demande_at'];
                                    $expire_dt      = new DateTime($reference_expiration);
                                    $maintenant_dt  = new DateTime();
                                    $diff_heures    = ($maintenant_dt->getTimestamp() - $expire_dt->getTimestamp()) / 3600;
                                ?>
                                <?php if (!$groupe['renouvelable']) : ?>
                                    <span class="mresa-delai-depasse">
                                        <i class="fa-solid fa-ban"></i> Déjà renouvelée une fois — renouvellement impossible
                                    </span>

                                <?php elseif ($diff_heures <= 5) : ?>
...
<div class="mresa-overlay" id="mresa-overlay-succes">
    ...
    <div class="mresa-modale">
        ...
        <h3>Nouvelle demande envoyée !</h3>

        <p class="mresa-succes-expire">
            Votre demande de renouvellement a été envoyée à la pharmacie.
