...
require_once 'canaux_recuperation.php';

$identifiant   = trim($_POST['phone_email'] ?? '');
$canal_choisi  = trim($_POST['canal'] ?? 'email'); // 'email' ou 'sms'
...
try {
    ...
    // ===== 6. Recherche du compte et envoi si tout est en ordre =====
    ...
    if ($user) {
        ...
        $canaux = canauxDisponibles($user);

        // Important : on ne peut PAS dire "aucun moyen de récupération" ici,
        // même si c'est vrai — ça révélerait à quelqu'un de mal intentionné
        // qu'un compte existe avec cet identifiant. Le message reste toujours
        // générique, quoi qu'il se passe. La bonne page pour prévenir
        // honnêtement un utilisateur qu'il n'a pas de canal renseigné, c'est
        // son profil une fois connecté — pas cet écran anonyme.
        $destinataire = $canaux[$canal_choisi] ?? null;

        if ($destinataire !== null && $canal_choisi === 'email') {
            ...
            $sql_insert  = "INSERT INTO reset_tokens (id_user, token, date_expiration, canal) VALUES (?, ?, ?, 'email')";
        ...
        // $canal_choisi === 'sms' : rien à faire pour l'instant, SMS pas encore
        // actif (SMS_ACTIF = false) — canauxDisponibles() ne renverra jamais
        // de clé 'sms' tant que ce sera le cas, donc $destinataire restera null.
    }
