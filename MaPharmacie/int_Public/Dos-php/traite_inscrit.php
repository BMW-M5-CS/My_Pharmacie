...
try {
    ...
    if (!empty($contact_recuperation)) {
        ...
        // On sait déjà lequel des deux c'est (grâce à la détection ci-dessus) :
        // autant le ranger tout de suite dans la bonne colonne, plutôt que de
        // le redeviner plus tard à chaque utilisation.
        if ($est_email) {
            $email_recuperation = $contact_recuperation;
        } else {
            $telephone_recuperation = $contact_recuperation;
        }
    }
    ...
    $sql = "INSERT INTO users(nom, prenom, phone_email, email_recuperation, telephone_recuperation, \"password\", \"role\") 
            VALUES (?, ?, ?, ?, ?, ?, 'client')";
    $stmt = $pdo->prepare($sql);
    $stmt->execute([$nom, $prenom, $phone_email, $email_recuperation, $telephone_recuperation, $mdp_securise]);
